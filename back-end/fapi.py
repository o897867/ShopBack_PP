...
# 最佳指标功能
from routers.best_indicator import router as best_indicator_router
app.include_router(
    best_indicator_router,
    tags=["Best Indicator"],
)
...
async def calculate_indicators(
    ...
    try:
        # Get candles data
        if symbol == "ETHUSDT":
            if not eth_data_manager:
                raise HTTPException(status_code=503, detail="ETH data service not available")

            if interval == "15m":
                # Aggregate to 15m and fetch
                eth_data_manager.fetcher.aggregate_to_15m()
                candles = eth_data_manager.fetcher.get_recent_candles_15m(limit)
            elif interval == "3m":
                # Use existing 3m ETH data
                candles = eth_data_manager.fetcher.get_recent_candles(limit)
            else:
                raise HTTPException(status_code=400, detail="Only 3m and 15m intervals are supported for ETHUSDT")
        else:
            # For other symbols, we would need to fetch from Binance API
            # For now, return error
            raise HTTPException(status_code=400, detail="Currently only ETHUSDT is supported")
